...
export interface PromoCode {
  code: string
  type: "ton" | "percent" | "case"  // "ton" = fixed reward, "percent" = bonus % on deposit, "case" = free case open
  reward: number            // TON granted (for type "ton")
  bonusPercent: number      // % bonus on deposit (for type "percent")
  caseId: string            // case id to open (for type "case"), e.g. "promo"
  maxUses: number
  uses: number
...
function sanitize(p: any): PromoCode | null {
  ...
  return {
    ...
    type,
    reward: type === "ton" ? Math.max(0, Number(p.reward) || 0) : 0,
    bonusPercent: type === "percent" ? Math.max(0, Math.min(1000, Number(p.bonusPercent) || 0)) : 0,
...
// Admin: replace the full list (create/deactivate/delete are all expressed as
// a new list from the panel).
...
export async function redeemPromo(rawCode: string, tgId: string): Promise<RedeemResult> {
  ...
  if (p.expiresAt != null && Date.now() > p.expiresAt) return { ok: false, error: "expired" }
  if (p.maxUses > 0 && p.uses >= p.maxUses) return { ok: false, error: "depleted" }
  if (p.redeemedBy.includes(String(tgId))) return { ok: false, error: "already" }

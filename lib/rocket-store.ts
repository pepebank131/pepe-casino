...
async function genCrashPoint(): Promise<number> {
  ...
  const u = Math.random() // fresh random for the crash value
...
export async function placeRocketBet(input: {
  ...
  photo: string
  amount: number
}): Promise<{ ok: boolean; error?: string; state?: RocketState }> {
  const round = await advanceGame()
  ...
  return { ok: true, state: publicState(round, bets, await crashHistory()) }
}
...
export async function cashoutRocketBet(
  userId: string,
): Promise<{ ok: boolean; error?: string; multiplier?: number; won?: number; state?: RocketState }> {
  const round = await advanceGame()
  ...
  // Guard: if we're effectively at/over the crash point, reject.
  ...
  if (!rows.length) return { ok: false, error: "no_active_bet" }
  const bet = toBet(rows[0])
  const bets = await betsForRound(round.id)
  return { ok: true, multiplier: m, won: bet.won ?? 0, state: publicState(round, bets, await crashHistory()) }
}

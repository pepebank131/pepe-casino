...
export function verifyInitData(initData: string): TgUser | null {
  ...
  // Freshness: 15 minutes (limits stolen initData replay window)
  const authDate = Number(params.get("auth_date") || 0)
  if (authDate && Date.now() / 1000 - authDate > 900) return null
 ...
 * - In production (NODE_ENV===production) or when a bot token is set: requires
 *   valid signed initData. Never falls back to an unsigned user payload.
 * - Unsigned fallback is only allowed when BOT_TOKEN is missing AND
 *   NODE_ENV !== 'production' (local/dev before bot setup).
 */
export function resolveUser(initData: string | null | undefined): TgUser | null {
  ...
  const allowUnsigned = !getBotToken() && process.env.NODE_ENV !== "production"
  if (allowUnsigned) {
    try {
      const params = new URLSearchParams(initData)
      const user = JSON.parse(params.get("user") || "{}")

...
import { CASES, FREE_CASE, DEPOSIT_CASE, REFERRAL_CASE, PROMO_CASE, normalizeCaseContents, type CaseDef } from "@/lib/casino-data"
...
function sanitizeCase(c: any): CaseDef | null {
  ...
  return {
    ...
    model: ["paid", "free", "deposit", "referral", "promo"].includes(c.model) ? c.model : undefined,
    cooldownMs: c.cooldownMs != null ? Math.max(0, Number(c.cooldownMs) || 0) : undefined,
    contents: normalizeCaseContents(Array.isArray(c.contents) ? c.contents : []),
  }
}

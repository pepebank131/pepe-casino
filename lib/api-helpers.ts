...
export async function getOrCreatePlayer(user: TgUser, referredBy?: string, ip?: string): Promise<PlayerRow> {
  ...
  if (existing.length > 0) {
    ...
    const patched: PlayerData = {
      ...
      // ref_by is set only at account creation — never attach a referrer later.
      ref_by: data.ref_by,
...
/** @deprecated Client snapshots are forbidden — economy is server-authoritative. */
export async function saveSnapshot(): Promise<never> {
  throw new Error("saveSnapshot disabled: client must not write balance/inventory")
}

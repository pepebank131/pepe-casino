...
export async function checkAdmin(): Promise<{ ok: boolean; adminId: string }> {
  const data = await post("/api/admin/me", { initData: getInitData() }).catch(() => null)
  return { ok: !!data?.ok, adminId: String(data?.adminId || "") }
}
...
export function syncPlayer(snapshot: {
  ton: number
  freeCaseAt: number | null
  depositCaseAt: number | null
  depositedSinceOpen: number
  ...
  return post("/api/sync", { initData: getInitData(), ...snapshot })
}
...
export async function loadAdminPlayers(adminId?: string, search?: string): Promise<AdminPlayersResult> {
  const qs = new URLSearchParams({ initData: getInitData() })
  if (adminId) qs.set("adminId", adminId)
...
export async function unbanAllPlayersApi(adminId?: string): Promise<number> {
  const data = await post("/api/admin/players", { initData: getInitData(), action: "unbanAll", adminId })
...
export async function loadPlayerDetail(uid: string, adminId?: string): Promise<AdminPlayerDetail> {
  const qs = new URLSearchParams({ initData: getInitData(), uid })
  if (adminId) qs.set("adminId", adminId)
...
export async function requestWithdrawal(input: {
  ...
  const data = await post("/api/admin/withdrawals", { initData: getInitData(), action: "create", ...input })
...
export async function placeRocketBet(amount: number): Promise<RocketState> {
...
export async function cashoutRocket(): Promise<{ multiplier: number; won: number; state: RocketState }> {
...
export async function payWithStars(opts: {
  ...
  // Required for deposits: TON to credit on success (echoed in the payload so
  // the bot webhook credits the right amount). Rate: 50 Stars = 0.3 TON.
  ton?: number
  // Required for withdrawals: the catalog NFT id the bot matches in inventory.
  nftId?: string
...
export async function resetSeason(adminId: string): Promise<{ season: { index: number; startMs: number; endMs: number; endsInMs: number } }> {
  return post("/api/admin/season", { adminId })
}

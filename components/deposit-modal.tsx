...
import { payWithStars, isTelegram, logDepositActivity, redeemPromo } from "@/lib/api-client"
...
export function DepositModal({ onClose }: { onClose: () => void }) {
  const { addTon, recordDeposit } = useStore()
  ...
  async function sendTon() {
    ...
    try {
      ...
      // Apply percent bonus if active
      const bonus = appliedBonus ? Math.round(n * (appliedBonus.percent / 100) * 1000) / 1000 : 0
      const total = Math.round((n + bonus) * 1000) / 1000
      addTon(total)
      recordDeposit(total)
      logDepositActivity(total, "ton")
      if (bonus > 0) {
        toast(t("deposit.depositedBonus", { n, b: bonus, p: appliedBonus!.percent, t: total }), "win")
        setAppliedBonus(null)
      } else {
        toast(t("deposit.success", { n: total }), "win")
      }
      onClose()
    } catch {
  ...
  async function payStars() {
    ...
    try {
      ...
      if (status === "paid") {
        ...
        addTon(total)
        recordDeposit(total)
        logDepositActivity(total, "stars")
        if (bonus > 0) toast(t("deposit.paidStarsBonusCelebrate", { n, t: total, b: bonus }), "win")
        else toast(t("deposit.paidStars", { n, t: total }), "win")
        setAppliedBonus(null)
        onClose()
  ...
  async function applyPromo() {
    ...
    try {
      ...
      if (res.type === "ton") {
        ...
        addTon(reward)
        recordDeposit(0)
        logDepositActivity(reward, "promo")
      ...
      toast(t("deposit.promoNetworkError"), "error")
    } finally {

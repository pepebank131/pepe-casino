...
import { getInitData, getTgUser, isTelegram, loadMaintenance } from "@/lib/api-client"
...
function BannedGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/check-ban", {
      ...
      .catch(() => { setBanned(true); setChecked(true) }) // fail closed
  }, [])
...
export function PepeApp() {
  ...
  useEffect(() => {
    let cancelled = false
    async function run() {
      // Server-side admin check (allowlist is never shipped to the client).
      if (isTelegram()) {
        try {
          ...
          if (!cancelled && res.ok && data.admin) {
            setGate("open")
            return
          }
        } catch {
        ...
        // Non-Telegram preview: still respect maintenance flag.
        void getTgUser
      }
      try {
        const on = await loadMaintenance()
        if (!cancelled) setGate(on ? "blocked" : "open")
      } catch {

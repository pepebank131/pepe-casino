...
import { checkAdmin } from "@/lib/api-client"
...
export function TopBar({ onDeposit }: { onDeposit: () => void }) {
  ...
  useEffect(() => {
    ...
    checkAdmin().then((res) => setIsAdmin(res.ok)).catch(() => setIsAdmin(false))
  }, [])
      ...
      <div className="flex items-center gap-2">
        {isAdmin && (
          <a
            ...
            className="flex h-9 items-center rounded-full px-3 text-xs font-extrabold transition active:scale-95"
            style={{

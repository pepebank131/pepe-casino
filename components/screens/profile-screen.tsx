...
import { requestWithdrawal, isTelegram, payWithStars, loadWithdrawFee, sellInventoryApi } from "@/lib/api-client"
...
export function ProfileScreen({ onDeposit }: { onDeposit: () => void }) {
  const { username, tgId, ton, inventory, photoUrl, addTon, removeFromInventory, setTon } = useStore()
  ...
  async function sell(item: InventoryItem) {
    ...
    if (isTelegram()) {
      try {
        const res = await sellInventoryApi(item.uid)
        setTon(res.ton)
        removeFromInventory(item.uid)
        toast(`Sold ${item.name} for ${res.soldFor} TON`, "win")
      } catch {
...
function WithdrawSheet({ item, fee, onClose, onSuccess }: { item: InventoryItem; fee: number; onClose: () => void; onSuccess: () => void }) {
  const { markWithdrawing, removeFromInventory } = useStore()
  const toast = useToast()
  ...
  async function confirm() {
    ...
    try {
      ...
      if (isTelegram()) {
        ...
        // Webhook creates the withdrawal after Stars payment.
        removeFromInventory(item.uid)
      } else {

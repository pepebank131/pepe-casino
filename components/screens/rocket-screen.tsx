...
export function RocketScreen() {
  const { ton, spendTon, addTon, addToInventory, inventory, removeFromInventory, username, photoUrl } = useStore()
  ...
  const placeBet = useCallback(async () => {
    ...
    if (mode === "nft") {
      ...
      // Remove the NFT from inventory immediately so it can't be double-spent.
  ...
  const doCashout = useCallback(async () => {
    ...
    try {
      if (inTelegram) {
        ...
        applyWin(res.won, res.multiplier)
        setState(res.state)
      } else {
        const m = liveMult

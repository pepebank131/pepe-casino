...
import { loadGlobalRtp, logUpgrade } from "@/lib/api-client"

export function UpgradeScreen({ onBack }: { onBack: () => void }) {
  const { inventory, removeFromInventory, addToInventory } = useStore()
  ...
  function upgrade() {
    ...
    setSpinning(true)
    const win = Math.random() * 100 < chanceFrozen

    // Wheel math:
    // - conic-gradient: green 0°→greenDeg, red greenDeg→360°
    // - pointer fixed at TOP reads segment at: (360 - totalAngle % 360) % 360
    // - WIN:  pointer must read [0, greenDeg)  → totalAngle % 360 ∈ (360-greenDeg, 360)
    // - LOSS: pointer must read [greenDeg, 360) → totalAngle % 360 ∈ [0, 360-greenDeg)
    const greenDeg = (chanceFrozen / 100) * 360
    const fullSpins = 360 * 5

    // We need to control where (currentAngle + fullSpins + extra) lands mod 360.
    // So: target = desired final (mod 360), extra = target - (currentAngle + fullSpins) mod 360
    // Then adjust to be positive.
    setAngle((currentAngle) => {
      const base = (currentAngle + fullSpins) % 360
      let target: number
      if (win) {
        // anywhere in (360-greenDeg, 360) — pointer over green
        target = (360 - greenDeg) + Math.random() * (greenDeg - 2) + 1
      } else {
        // anywhere in (1, 360-greenDeg-1) — pointer over red
        target = 1 + Math.random() * (360 - greenDeg - 2)
      }
      // extra must bring base → target, staying positive

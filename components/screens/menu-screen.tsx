...
export function MenuScreen({ onUpgrade }: { onUpgrade: () => void }) {
  ...
  async function redeem() {
    ...
    setRedeeming(true)
    try {
      // In Telegram, validate + record redemption server-side (codes are admin-managed).
      if (isTelegram()) {
        ...
        // Local demo mode (outside Telegram): keep the built-in sample codes.
        if (code === "PEPE100") {
          addTon(1)
